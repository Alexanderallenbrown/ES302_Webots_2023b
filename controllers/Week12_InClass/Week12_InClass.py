...
import cv2 #import openCV for functions.

#create a file to write results to:
f = open('Week12_Data.txt','w')
...
#get camera
camera = romi.simromi.getDevice("camera")
...
while romi.simromi.step(timestep) != -1:
    ...
    FSM.update((proxReading>500),T0.state,T1.state)
    #now block 4: do what needs to be done in each state
    if(FSM.FWD):
        romi.update(100,100,80,0,70)
    elif(FSM.TURN):
        romi.update(100,-100,80,0,70)
    elif(FSM.WAIT):
        #stop
        romi.update(0,0,80,0,70)
    else:
