...
####### IMPORTANT!! IN COMPLETING THE SECOND SIMULATION FOR A09, SET THIS TO TRUE. ELSE, SET TO FALSE ######
generateWorldUsingMapImage = False
...
f.write("time, robot_x, robot_y, pingpong_x, pingpong_y \r\n")

# get access to our two robots in the world
myromi = robot.getFromDef('Romi')
pingpong = robot.getFromDef('PingPong')
...
while robot.step(timestep) != -1:
  ...
  ball_pos = pingpong.getPosition()
  #write them to file in the specified format
  f.write(str(simtime)+","+str(myromi_pos[0])+","+str(myromi_pos[1])+","+str(ball_pos[0])+","+str(ball_pos[1])+"\r\n")
  #update simtime
  simtime+=timestep
